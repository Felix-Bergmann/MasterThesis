                ...
                for (int j = 0; j < i; j++) {
                    for (int k = 0; k < j; k++) {
                        for (int l = 0; l < k; l++) {
                            ...
                            int w = notElim.get(k);
                            int x = notElim.get(l);
                            if((utilities[u][v]+utilities[u][w]+utilities[u][x]) > curr_utility[u] && 
                               (utilities[v][u]+utilities[v][w]+utilities[v][x]) > curr_utility[v] && 
                               (utilities[w][u]+utilities[w][v]+utilities[w][x]) > curr_utility[w] && 
                               (utilities[x][u]+utilities[x][v]+utilities[x][w]) > curr_utility[x]){
