                    ...
                    for (int k = 0; k < j; k++) {
                        for (int l = 0; l < k; l++) {
                            for (int m = 0; m < l; m++) {
                                ...
                                int x = notElim.get(l);
                                int y = notElim.get(m);
                                if((utilities[u][v]+utilities[u][w]+utilities[u][x]+utilities[u][y]) / 5.0 > curr_utility[u] && 
                                   (utilities[v][u]+utilities[v][w]+utilities[v][x]+utilities[v][y]) / 5.0 > curr_utility[v] && 
                                   (utilities[w][u]+utilities[w][v]+utilities[w][x]+utilities[w][y]) / 5.0 > curr_utility[w] && 
                                   (utilities[x][u]+utilities[x][v]+utilities[x][w]+utilities[x][y]) / 5.0 > curr_utility[x] &&
                                   (utilities[y][u]+utilities[y][v]+utilities[y][w]+utilities[y][x]) / 5.0 > curr_utility[y]){
